...
import { Wallet as WalletImpl, initHoosatFramework } from "@hoosat/wallet";
import { TxSend, TxInfo } from "@hoosat/wallet/types/custom-types";
...
export class WorkerCore extends EventEmitter {
  ...
  async init() {
    ...
    super.init();
    workerLog.debug("before initHoosatFramework");
    await initHoosatFramework();
    workerLog.debug("after initHoosatFramework");
  ...
  initWalletHanler() {
    ...
    this.on("wallet-request", async (msg: { fn: string; rid: string; args: any[] }) => {
      let { fn, rid, args } = msg;
      let { wallet } = this;
      if (!wallet) return this.sendWalletResponse(rid, `Wallet not initilized yet. (subject:${fn})`);
      if (!fn) return this.sendWalletResponse(rid, "Invalid wallet request.");
