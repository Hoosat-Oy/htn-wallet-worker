...
import { Wallet, EventTargetImpl, helper, hoosatcore, CONFIRMATION_COUNT, COINBASE_CFM_COUNT } from "@hoosat/wallet";
const { HDPrivateKey } = hoosatcore;
...
export const initHoosatFramework = (opt: { workerPath?: string } = {}) => {
  return new Promise<void>((resolve, reject) => {
    helper.dpc(2000, () => {
      ...
      if (IS_NODE_CLI) {
        ...
        let { workerPath = "/node_modules/@hoosat/wallet-worker/worker.js" } = opt;
        url = new URL(workerPath, baseURL);
      }
      workerLog.info("initHoosatFramework", url, baseURL);
...
class WalletWrapper extends EventTargetImpl {
  ...
  uid: string;
  HDWallet: hoosatcore.HDPrivateKey;
  ...
  constructor(privKey: string, seedPhrase: string, networkOptions: NetworkOptions, options: WalletOptions = {}) {
    ...
    if (privKey && seedPhrase) {
      this.HDWallet = new hoosatcore.HDPrivateKey(privKey);
    } else {
      const temp = new Wallet.Mnemonic(Wallet.Mnemonic.Words.ENGLISH);
      this.HDWallet = new hoosatcore.HDPrivateKey(temp.toHDPrivateKey().toString());
    }
  ...
  checkGRPCFlags() {
    ...
    rpc.onConnect(async () => {
      ...
      let result = await rpc.getUtxosByAddresses([]).catch((err) => {
        //error = err;
      });
  ...
  initWorker() {
    if (!worker) throw new Error("Please init hoosat framework using 'await initHoosatFramework();'.");
    this.worker = worker;
  ...
  async handleRPCRequest(msg: { fn: string; args: any; rid?: string }) {
    ...
    let directFns = ["onConnect", "onDisconnect", "onConnectFailure", "onError", "disconnect", "connect"];
    ...
    let { uid: subUid } = p;
    let error;
    let result = await p.catch((err: any) => {
      error = err;
    });
  ...
  async sync(syncOnce: boolean | undefined = undefined) {
    ...
    let args = [];
    if (syncOnce !== undefined) args.push(syncOnce);
  ...
  get(name: string, waitForSync: boolean = false) {
    return new Promise(async (resolve, reject) => {
      if (waitForSync) await this.syncSignal;
   ...
   * @param txParams
   * @param txParams.toAddr To address in cashaddr format (e.g. hoosattest:qq0d6h0prjm5mpdld5pncst3adu0yam6xch4tr69k2)
   * @param txParams.amount Amount to send in sompis (100000000 (1e8) sompis in 1 HTN)
